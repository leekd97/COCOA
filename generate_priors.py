...
@torch.no_grad()
def compute_entity_prior(model, tokenizer, entity: str, device, bos: str,
                         mode: str = "sum") -> float:
    """Compute log P(entity | BOS). Returns sum or mean over tokens by mode."""
    ...
    if mode == "mean" and n_tokens > 0:
        return total_lp / n_tokens
    return total_lp


@torch.no_grad()
def compute_priors_batched(model, tokenizer, entities: List[str], device, bos: str,
                           batch_size=32, mode: str = "sum") -> Dict[str, float]:
    """Compute priors for a list of entities, batched. Sum or mean per `mode`."""
    ctx_enc = tokenizer(bos, return_tensors="pt", add_special_tokens=True)
    ctx_len = ctx_enc["input_ids"].size(1)
...
def generate_priors_for_culture(
    ...
    LOG.info(f"Generating priors: {culture}/{lang} [mode={mode}]")
    ...
    for etype, ent_dict in data.entities.items():
        ...
        for side in ["asian", "western"]:
            ...
            priors[etype][side] = compute_priors_batched(
                model, tokenizer, ent_list, device, bos, mode=mode
            )
...
def main():
    ...
    parser.add_argument("--output_root", default=None,
                        help="Output root. Defaults: ./dataset/priors (sum) or ./dataset/priors_mean (mean)")
    parser.add_argument("--mode", choices=["sum", "mean"], default="sum",
                        help="Aggregation mode for entity log-prob")
    parser.add_argument("--device", default="cuda:0")
    args = parser.parse_args()
    
    # Resolve default output_root by mode (preserves backward compatibility)
    if args.output_root is None:
        args.output_root = "./dataset/priors_mean" if args.mode == "mean" else "./dataset/priors"
    LOG.info(f"Mode: {args.mode}, output: {args.output_root}")
