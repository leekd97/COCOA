...
# Data Split (context + entity split)
def split_data(
    data: CamelliaData,
...
# Paired Context Dataset (same entity, different contexts)
@dataclass
class CategoryData:
...
class PairedDataset(Dataset):  
    def __init__(
        ...
        for cat_key, cdata in category_data.items():
            ...
            if pairing == "nxm":
                # All NxM combinations (shuffled, capped)
                ...
                # NxN: equal-sized sides, all combinations
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # Unpaired mode: sample different entity pair for neutral (same category)
        if self.paired:
            n_asian, n_western = asian, western
        else:
            available_a = [i for i in range(len(cdata.asian_entities)) if i != a_idx]
            available_w = [i for i in range(len(cdata.western_entities)) if i != w_idx]
            n_a_idx = random.choice(available_a)
            n_w_idx = random.choice(available_w)
            n_asian = cdata.asian_entities[n_a_idx]
            n_western = cdata.western_entities[n_w_idx]
        
        # Build 4 sequences
        g_asian_text = g_ctx.replace("[MASK]", asian)
        g_western_text = g_ctx.replace("[MASK]", western)
        n_asian_text = n_ctx.replace("[MASK]", n_asian)
        n_western_text = n_ctx.replace("[MASK]", n_western)
        ...
        result = {
            ...
            # Entity strings (for prior lookup — grounded side)
            "asian_entity": asian,
            "western_entity": western,
        }
        
        # Unpaired: neutral uses different entities, need separate prior lookup
        if not self.paired:
            result["n_asian_entity"] = n_asian
            result["n_western_entity"] = n_western
...
def create_paired_dataloader(
    ...
    print(f"\nPaired Dataset (per category, pairing={pairing}, paired={paired}):")
    ...
    dataset = PairedDataset(cat_data, tokenizer, pairs_per_category, max_length, seed, pairing, paired)
...
# Dataset (kept for evaluation / backward compat)
...
def create_balanced_dataloader(
    ...
    dataset = CoCoADataset(examples, tokenizer, max_length)
