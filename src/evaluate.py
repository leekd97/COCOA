# CoCoA Evaluation Module
"""
Handles:
- CBS (Cultural Bias Score) computation
- N x M robust evaluation (all local x all other-culture comparisons)
- Batched inference for speed

Completion log-probabilities are summed over the entity's tokens (Eq. 1).
"""
...
def compute_cbs_for_context_robust(
    ...
    if entity_priors is not None and prior_alpha != 0.0:
        asian_logprobs = [
            lp - prior_alpha * entity_priors.get(ent, 0.0)
...
def evaluate_robust_fair(
    ...
    show_progress: bool = True,
    entity_priors=None,
    prior_alpha_g: float = 1.0,
    prior_alpha_n: float = 1.0,
) -> Dict:
    _orig_use_cache = getattr(model.config, "use_cache", True)
    try:
        model.config.use_cache = False
        return _evaluate_robust_fair_impl(
            model=model,
            tokenizer=tokenizer,
            ...
            prior_alpha_g=prior_alpha_g,
            prior_alpha_n=prior_alpha_n,
        )
    finally:
        model.config.use_cache = _orig_use_cache


def _evaluate_robust_fair_impl(
    model: PreTrainedModel,
    tokenizer: PreTrainedTokenizer,
    split_info: Dict,
    entities: Dict,
