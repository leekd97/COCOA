#!/bin/bash
# Exp 2: PMI scaled on neutral (alpha_g=1.0, alpha_n=0.3)
cd "$(dirname "$0")/.."
...
K=5
SEED=45
EXP_SUBDIR="kfold_pnorm_scaled_${ALPHA_N}"
...
for FOLD in $(seq 0 $((K-1))); do
    ...
    EXP_NAME="${CULTURE}_${LANG}_${MODEL_SHORT}_${NEUTRAL_LOSS}_wg${W_GROUNDED}_wn${W_NEUTRAL}_tau${TAU}_r${LORA_R}_pnorm-g1.0-n${ALPHA_N}_fold${FOLD}_seed${SEED}"
    ...
    echo "[$COUNT/$TOTAL] RUN ${CULTURE} fold${FOLD} (α_n=$ALPHA_N)"
    START_TIME=$(date +%s)
    ...
    python main.py \
        ...
        --prior_alpha_g 1.0 --prior_alpha_n $ALPHA_N \
