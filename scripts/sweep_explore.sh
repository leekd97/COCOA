#!/bin/bash
# Llama VI: alpha_n exploration, fold 0
cd "$(dirname "$0")/.."
...
CONFIGS=(
    "vi_an1.0_d0:1.0:1.0:0:1.0:2.0:10.0"
    "vi_an0.7_d0:1.0:0.7:0:1.0:2.0:10.0"
    "vi_an1.0_d1:1.0:1.0:1:1.0:2.0:10.0"
)

TOTAL=${#CONFIGS[@]}
COUNT=0

echo "[$(date)] Llama VI alpha_n exploration: $TOTAL configs"
...
for CFG in "${CONFIGS[@]}"; do
    ...
    EXP_NAME="vi_${LANG}_${MODEL_SHORT}_${NEUTRAL_LOSS}_wg${W_GROUNDED}_wn${W_NEUTRAL}_tau${TAU}_r${LORA_R}_pnorm-g${AG}-n${AN}_nxn_fold${FOLD}_seed${SEED}"
    ...
    python main.py \
        --culture vi --lang "$LANG" --model "$MODEL" \
        ...
        2>&1 | tee "$OUTPUT/$EXP_SUBDIR/vi_${MODEL_SHORT}_fold${FOLD}.log"
