#!/bin/bash
# CoCoA K-Fold Sweep (Llama)
# Usage: tmux new -s kfold_llama 'bash scripts/sweep_kfold_llama.sh'
...
echo "[$(date)] K-Fold sweep started (PID=$$)"

TOTAL=$(( ${#CULTURES[@]} * K ))
COUNT=0
SKIP=0

echo "=== CoCoA K-Fold Sweep (${MODEL_SHORT}, ${LANG}) ==="
...
for FOLD in $(seq 0 $((K-1))); do
    ...
    EXP_NAME="${CULTURE}_${LANG}_${MODEL_SHORT}_${NEUTRAL_LOSS}_wg${W_GROUNDED}_wn${W_NEUTRAL}_tau${TAU}_r${LORA_R}_fold${FOLD}_seed${SEED}"
    ...
    python main.py \
        ...
        --eval_steps 200 \
        --log_steps 50 \
        2>&1 | tee "$OUTPUT/${CULTURE}_${LANG}_${MODEL_SHORT}_fold${FOLD}.log"
