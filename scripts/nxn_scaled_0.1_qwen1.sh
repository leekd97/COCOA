#!/bin/bash
# Exp C: N×N + PMI scaled (α_g=1.0, α_n=0.1) — conservative
cd "$(dirname "$0")/.."
...
K=5
SEED=45
EXP_SUBDIR="kfold_nxn_scaled_0.1"
...
for FOLD in $(seq 0 $((K-1))); do
    COUNT=$((COUNT + 1))
    EXP_NAME="${CULTURE}_${LANG}_${MODEL_SHORT}_${NEUTRAL_LOSS}_wg${W_GROUNDED}_wn${W_NEUTRAL}_tau${TAU}_r${LORA_R}_pnorm-g1.0-n0.1_nxn_fold${FOLD}_seed${SEED}"
    ...
    python main.py \
        ...
        --prior_alpha_g 1.0 --prior_alpha_n 0.1 \
        --pairing nxn \
    ...
    if [ $EXIT_CODE -eq 0 ]; then
        echo "[$(date)] OK: ${CULTURE} fold${FOLD} ${ELAPSED}s"
    elif [ $EXIT_CODE -eq 137 ]; then
        FAIL=$((FAIL+1)); echo "[$(date)] ★ OOM KILLED (137): ${CULTURE} fold${FOLD}"
    elif [ $EXIT_CODE -eq 139 ]; then
        FAIL=$((FAIL+1)); echo "[$(date)] ★ SEGFAULT (139): ${CULTURE} fold${FOLD}"
    else
        FAIL=$((FAIL+1)); echo "[$(date)] FAILED ($EXIT_CODE): ${CULTURE} fold${FOLD} ${ELAPSED}s"
    fi
